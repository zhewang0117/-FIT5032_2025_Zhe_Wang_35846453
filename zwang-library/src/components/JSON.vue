...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>Finding by property: <span :class="{ 'highlight': orwell && orwell.name }">{{ orwell?.name }}</span></p>
    ...
    <section class="lab-section">
      ...
      <li v-for="(opens, type) in bookstores.openingHours" :key="type">
         {{type}} : {{opens.open}}
      </li>
      ...
      <p>Our #1 seller:</p> {{bookstores.topSellers[0]}}
    </section>
...
const originalText = 'Finding by property: {{ orwell?.name }}';
const highlightedText = computed(() => {
  return originalText.replace(/(\{\{.*?\}\})/g, '<span class="highlight">$1</span>');
});
